...
static int mfs_rm_dir(char *name)
{
	int res;

	printf("borrar el directorio '%s'\n", name);

	res = mfs_rmdir(name);

	if (res == -1) {
       		printf("No puedo berror el directorio '%s'. Error %s\n",
		       name, strerror(errno));
		exit(-4);
	}
...
int main (int argc, char **argv)
{
	...
	if (argc == optind) {
		printf ("Necesito los directorios a borrar\n\n");
		usage(-1);
	}
	while (optind < argc)
		mfs_rm_dir(argv[optind++]);
