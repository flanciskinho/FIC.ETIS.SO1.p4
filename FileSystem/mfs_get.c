...
{
	printf(
		"Usage:  mfs_get [OPTION] ORIGEN DEST\n"
...
static int mfs_get(char *source, char *target)
{
	int in;
	int out;
	char * buffer;
	...
	if (fchmod(out, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP) != 0) {
		printf("Error dando permisos af fichero '%s'\n", target);
		exit(-4);
	}
...
int main (int argc, char **argv)
{
	...
	mfs_get(argv[optind], argv[optind+1]);
